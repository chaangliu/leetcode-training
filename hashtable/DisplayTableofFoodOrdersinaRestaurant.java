...
public class DisplayTableofFoodOrdersinaRestaurant {
    /**
     * 题意：根据点菜名单，确定每桌需要上的菜
     * 直接归类就行，要注意set中的字符串并不是字典序排列的
     */
    public List<List<String>> displayTable(List<List<String>> orders) {
        HashMap<String, HashMap<String, Integer>> map = new HashMap<>(); // table -> [dish, count]
        HashSet<String> dishSet = new HashSet<>();
        for (List<String> list : orders) {
            String table = list.get(1), dish = list.get(2);
            dishSet.add(dish);
        ...
        Collections.sort(dishList, (s1, s2) -> s1.compareTo(s2)); // set中的字符串并不是字典序排列的
        dishList.add(0, "Table");
        res.add(dishList);
        List<String> tables = new ArrayList<>(map.keySet());
        Collections.sort(tables, (a, b) -> Integer.parseInt(a) - (Integer.parseInt(b)));
        for (String table : tables) {
            List<String> l = new ArrayList<>();
            l.add(table);
            Map<String, Integer> dishCnt = map.get(table);
            for (int i = 1; i < dishList.size(); i++) {
                String dish = dishList.get(i);
                l.add(dishCnt.getOrDefault(dish, 0) + "");
            }
            res.add(l);
        }
        return res;
